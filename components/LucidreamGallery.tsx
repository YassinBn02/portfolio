...
const getImageUrl = (image: string) => {
  // If the image is already encoded, decode it first to avoid double-encoding
  const decodedImage = decodeURIComponent(image);
  // Encode each part of the path separately
  const encodedImage = decodedImage.split('/')
    .map(part => encodeURIComponent(part))
    .join('/');
  return `/Lucidream/${encodedImage}`;
};
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[90vw] max-h-[90vh] p-0 bg-transparent border-none">
        <DialogTitle className="sr-only">Lucidream Gallery</DialogTitle>
...
export default function LucidreamGallery() {
  ...
  const projectInfo = {
    title: "LUCIDREAM",
    year: "2023",
    creator: "Marwen Ben Cheikh",
    role: "Costume Designer",
  };

  // List of images from the Lucidream folder with original filenames
  const lucidreamImages = [
    '1.png',
    '10.png',
    '15.png',
    '27.png',
  ...
  return (
    <div className="container mx-auto px-4">
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {lucidreamImages.map((image, index) => (
          <div
            key={image}
